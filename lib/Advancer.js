...
export default class Advancer extends EventEmitter {
    ...
    _processWorkerResult(workerResult) {

        if (
            workerResult.hasOwnProperty('payload') &&
            workerResult.hasOwnProperty('resolution')
        ) {
            return {
                payload: workerResult.payload,
                resolution: workerResult.resolution
            };
        }

        if (workerResult.hasOwnProperty('_resolution')) {
            return {
                resolution: workerResult._resolution,
                payload: r_omit(['_resolution'], workerResult)
            };
        }
        ...
        return this._exchange.getMessage(srcQueue)
            ...
            .then((srcMessage) => {
                ...
                return this._callWorker(srcQueue, srcMessage.payload)
                    .then((workerResult) => {
                        var newMessage = this._processWorkerResult(workerResult);
